...
int x11SendKey(KeySym key)
{
    Display* disp = XOpenDisplay(NULL);
    if(disp == NULL)
        return 1;

    KeyCode kcode = XKeysymToKeycode(disp, key);

    // 按下
    XTestFakeKeyEvent(disp, kcode, True, CurrentTime);
    // 释放
    XTestFakeKeyEvent(disp, kcode, False, CurrentTime);
